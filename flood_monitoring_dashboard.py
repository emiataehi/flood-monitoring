...
class FloodMonitoringDashboard:
    ...
    def fetch_river_data(self, days_back=7):
        ...
        for station_name, measure_id in stations.items():
            url = (
                ...
                f"{measure_id}/readings?_sorted&_limit=500"
            )
            try:
    ...
    def _generate_sample_data(self, days_back=90):
        ...
        for station in stations:
            base_level = base_levels.get(station, 0.2)
            for date in dates:
        ...
        if data is not None:
            ...
            for i, station in enumerate(data['location_name'].unique()):
                with cols[i]:
                    ...
                    if self.ci_calculator:
                        ci_data = self.ci_calculator.calculate_interval(data, station)
                    else:
                        ci_data = {'confidence_percentage': 0, 'margin_of_error': 0, 'lower_bound': 0, 'upper_bound': 0}

                    with st.expander(f"{station} Prediction Details", expanded=True):
                        st.metric(
                            "Current Level",
                            f"{current_level:.3f}m" if current_level is not None else "N/A",
                            f"Risk: {risk_level}",
            ...
            if self.anomaly_detector:
                ...
                if anomaly_summary:
                    ...
                    if st.checkbox("Show Detailed Anomalies"):
                        for station in anomaly_summary.keys():
            ...
            for i, station in enumerate(data['location_name'].unique()):
                with cols[i]:
                    ...
                    fig = go.Figure(go.Indicator(
                        ...
                        value=current_level if current_level is not None else 0,
                        domain={'x': [0, 1], 'y': [0, 1]},
    ...
    def show_station_details(self, data):
        ...
        selected_station = st.selectbox("Select Station", list(STATION_CONFIG.keys()))
        ...
        with col1:
            ...
            if data is not None:
                ...
                for station in data['location_name'].unique():
                    ...
                    stations_df.loc[mask, 'Current Level'] = f"{current_level:.3f}m" if current_level is not None else "N/A"
    ...
    def show_alerts(self, data):
        ...
        for station in data['location_name'].unique():
            ...
            if risk_level == 'NORMAL' and recent_trend > 0 and current_level is not None:
        ...
        for i, alert_info in enumerate(alert_summary):
            with cols[i]:
                ...
                level_display = f"{alert_info['current_level']:.3f}m" if alert_info['current_level'] is not None else "N/A"
                ...
                <div style='padding: 20px; border-radius: 10px; border: 2px solid {alert_info['color']}; background-color: {alert_info['color']}20;'>
                    ...
                    <p><strong>Level:</strong> {level_display} {alert_info['trend']}</p>
    ...
    def generate_report(self, data):
        ...
        total_stations = data['location_name'].nunique()
        ...
        for station in data['location_name'].unique():
            ...
            if risk_level in ['HIGH', 'MODERATE']:
                ...
                normal_stations.append(station)

        with col1:
            st.metric("Total Stations Monitored", total_stations)
        with col2:
            st.metric("Stations at Risk", len(high_risk_stations),
                      delta="Critical" if len(high_risk_stations) > 0 else "Normal",
                      delta_color="inverse" if len(high_risk_stations) > 0 else "off")
        with col3:
            st.metric("Data Points (7 days)", f"{total_readings:,}")
        ...
        for station in data['location_name'].unique():
            ...
            recent_24h = station_data.head(24)
            avg_24h = recent_24h['river_level'].mean()
            max_24h = recent_24h['river_level'].max()
            trend = recent_24h['river_level'].diff().mean()

            status_table.append({
                'Station': station,
                'Current Level (m)': f"{current_level:.3f}" if current_level is not None else "N/A",
                'Risk Status': risk_level,
                '24h Average (m)': f"{avg_24h:.3f}",
                '24h Peak (m)': f"{max_24h:.3f}",
        ...
        with col1:
            ...
            st.caption(f"Includes {len(data)} readings from {data['location_name'].nunique()} stations")

        with col2:
            st.markdown("**📄 Summary Report**")

            summary_report = f"""Flood Monitoring Summary Report
Generated: {report_time}

SYSTEM STATUS
{'=' * 50}
Total Stations: {total_stations}
Stations at Risk: {len(high_risk_stations)}
Normal Operations: {len(normal_stations)}

STATION DETAILS
{'=' * 50}
"""
            for station_info in status_table:
                summary_report += f"\n{station_info['Station']}:"
                summary_report += f"\n  Current Level: {station_info['Current Level (m)']}m"
                summary_report += f"\n  Status: {station_info['Risk Status']}"
                summary_report += f"\n  Trend: {station_info['Trend']}\n"

            if high_risk_stations:
                summary_report += f"\n\nALERT: The following stations require attention:\n"
                for station in high_risk_stations:
                    summary_report += f"  - {station}\n"
            else:
            ...
            st.caption("Text-based summary for quick reference")
    ...
    def show_mobile_dashboard(self, data):
        ...
        for station in data['location_name'].unique():
            ...
            level_display = f"{current_level:.3f}m" if current_level is not None else "N/A"
        ...
        overall_risk = max(system_risks, key=lambda x: risk_priority[x]) if system_risks else "LOW RISK"
        ...
        if overall_risk == "HIGH RISK":
            st.error("### 🚨 CRITICAL ALERT\nHigh-risk conditions detected. Immediate action may be required.")
        elif overall_risk == "MODERATE RISK":
            st.warning("### ⚠️ FLOOD WARNING\nModerate risk conditions present. Stay alert and prepared.")
        elif overall_risk == "CAUTION":
            st.warning("### ⚠️ CAUTION LEVEL\nPotential flood risk detected. Monitor conditions closely.")
